functions {
  ...
  real partial_sum2_lpdf(array[] real y,int start, int end,vector mu,real phi, int type){
    real out;
    if(type==1) out = gamma_lpdf(y[1:(1+end-start)]|1.0/phi, 1.0./(phi*mu[1:(1+end-start)]));
    if(type==2) out = gamma_lpdf(y[1:(1+end-start)]|1.0/phi, mu[1:(1+end-start)]/phi);
    if(type==3) out = gamma_lpdf(y[1:(1+end-start)]|1.0/phi, 1.0./(phi*log(mu[1:(1+end-start)])));
    return out;
  }
}
data {
