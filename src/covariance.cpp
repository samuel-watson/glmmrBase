...
glmmr::Covariance::Covariance(const str& formula,
                                     const ArrayXXd &data,
...
glmmr::Covariance::Covariance(const glmmr::Formula& form,
                                     const ArrayXXd &data,
                                     const strvec& colnames) :
...
glmmr::Covariance::Covariance(const glmmr::Covariance& cov) : form_(cov.form_), data_(cov.data_),
...
void glmmr::Covariance::Z_constructor()
{
  matZ.n = data_.rows();
  matZ.m = Q_;
...
void glmmr::Covariance::linear_predictor_ptr(glmmr::LinearPredictor* ptr){
  linpred_ptr = ptr;
  if(z_requires_update)Z_updater();
}

void glmmr::Covariance::Z_updater(){
...
void glmmr::Covariance::update_parameters_in_calculators(){
  for(int i = 0; i < B_; i++){
...
MatrixXd glmmr::Covariance::D(bool chol, bool upper)
{
  MatrixXd D(Q_,Q_);
  if(isSparse){
...
int glmmr::Covariance::npar() const
{
  return npars_;
};


int glmmr::Covariance::B() const
{
  return B_;
}

int glmmr::Covariance::Q() const
{
  if(Q_==0)throw std::runtime_error("Random effects not initialised");
  return Q_;
}

int glmmr::Covariance::max_block_dim() const
{
  int max = 0;
...
int glmmr::Covariance::block_dim(int b) const
{
  return block_size[b];//re_data_[b].rows();
};

intvec glmmr::Covariance::parameter_fn_index() const
{
  return re_fn_par_link_;
}

intvec glmmr::Covariance::re_count() const
{
  return re_count_;
}

void glmmr::Covariance::update_parameters(const dblvec& parameters)
{
...
void glmmr::Covariance::update_parameters_extern(const dblvec& parameters)
{
...
void glmmr::Covariance::update_parameters(const ArrayXd& parameters)
{
  if(parameters_.size()==0){
...
double glmmr::Covariance::get_val(int b, int i, int j) const
{
  return calc_[b].calculate<CalcDyDx::None>(i,j,0,0)[0];
}

MatrixXd glmmr::Covariance::get_block(int b)
{
...
MatrixXd glmmr::Covariance::Z()
{
  Z_updater();
  return sparse_to_dense(matZ,false,true);
}

MatrixXd glmmr::Covariance::get_chol_block(int b,bool upper)
{
...
VectorXd glmmr::Covariance::sim_re()
{
...
MatrixXd glmmr::Covariance::D_builder(int b, bool chol, bool upper)
{
  if (b == B_ - 1) {
...
sparse glmmr::Covariance::Z_sparse() {
  Z_updater();
  return matZ;
}

MatrixXd glmmr::Covariance::ZL() {
  sparse ZD = ZL_sparse();
...
MatrixXd glmmr::Covariance::ZLu(const MatrixXd& u){
  sparse ZL = ZL_sparse();
...
MatrixXd glmmr::Covariance::Lu(const MatrixXd& u){
#if defined(ENABLE_DEBUG) && defined(R_BUILD)
  Rcpp::Rcout << "\nLu() fn";
#endif
...
double glmmr::Covariance::log_likelihood(const VectorXd &u){
...
double glmmr::Covariance::log_determinant(){
...
void glmmr::Covariance::make_sparse(){
...
void glmmr::Covariance::L_constructor(){
...
void glmmr::Covariance::set_sparse(bool sparse, bool amd){
  use_amd_permute = amd;
  isSparse = sparse;
  if(sparse)make_sparse();
}

void glmmr::Covariance::update_ax(){
  int llim = 0;
  int nj = 0;
...
MatrixXd glmmr::Covariance::D_sparse_builder(bool chol,
                                                    bool upper){
#if defined(R_BUILD) && defined(ENABLE_DEBUG)
  Rcpp::Rcout << "\nD sparse builder";
#endif
...
bool glmmr::Covariance::any_group_re() const{
  bool gr = false;
  for(int i = 0; i < fn_.size(); i++){
    for(int j = 0; j < fn_[i].size(); j++){
      if(fn_[i][j]==CovFunc::gr){
...
strvec glmmr::Covariance::parameter_names(){
  strvec parnames;
...
VectorXd glmmr::Covariance::log_gradient(const MatrixXd &umat, double& logl){
...
void glmmr::Covariance::derivatives(std::vector<MatrixXd>& derivs,
                                           int order){
