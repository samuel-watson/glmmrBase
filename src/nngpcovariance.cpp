...
glmmr::nngpCovariance::nngpCovariance(const glmmr::Formula& formula,
                                             const ArrayXXd &data,
...
glmmr::nngpCovariance::nngpCovariance(const glmmr::nngpCovariance& cov) : Covariance(cov.form_, cov.data_, cov.colnames_, cov.parameters_),  
...
void glmmr::nngpCovariance::parse_grid_data(const ArrayXXd &data)
{
...
void glmmr::nngpCovariance::gen_NN(int nn){
  m = nn;
  A.conservativeResize(nn,NoChange);
  grid.genNN(m);
}

MatrixXd glmmr::nngpCovariance::D(bool chol, bool upper){
...
MatrixXd glmmr::nngpCovariance::ZL(){
  MatrixXd L = D(true,false);
  return L;
}

MatrixXd glmmr::nngpCovariance::LZWZL(const VectorXd& w){
...
MatrixXd glmmr::nngpCovariance::ZLu(const MatrixXd& u){
  MatrixXd L = D(true,false);
  MatrixXd ZLu = L * u;
  return ZLu;
}

MatrixXd glmmr::nngpCovariance::Lu(const MatrixXd& u){
  MatrixXd L = D(true,false);
  return L*u;
}

VectorXd glmmr::nngpCovariance::sim_re(){
...
sparse glmmr::nngpCovariance::ZL_sparse(){
  MatrixXd L = D(true,false);
  return dense_to_sparse(L);
}

int glmmr::nngpCovariance::Q() const {
  return grid.N;
}

double glmmr::nngpCovariance::log_likelihood(const VectorXd &u){
...
double glmmr::nngpCovariance::log_determinant(){
  return Dvec.array().log().sum();
}

void glmmr::nngpCovariance::gen_AD()
{
  A.setZero();
  Dvec.setZero();
...
VectorMatrix glmmr::nngpCovariance::submatrix(int i)
{
  int idxlim = i <= m ? i : m;
  double val = Covariance::get_val(0,0,0);
  Dvec(0) = val;
...
void glmmr::nngpCovariance::update_parameters(const dblvec& parameters)
{
...
void glmmr::nngpCovariance::update_parameters_extern(const dblvec& parameters)
{
  parameters_ = parameters;
  update_parameters_in_calculators();
  gen_AD();
...
void glmmr::nngpCovariance::update_parameters(const ArrayXd& parameters)
{
...
void glmmr::nngpCovariance::update_parameters_d(const ArrayXd& parameters)
{
  if(parameters_.size()==0){
    for(int i = 0; i < parameters.size(); i++){
      parameters_.push_back(parameters(i));
...
MatrixXd glmmr::nngpCovariance::inv_ldlt_AD(const MatrixXd &A, 
                                                   const VectorXd &D,
                                                   const ArrayXXi &NN)
{
  int n = A.cols();
...
void glmmr::nngpCovariance::gen_AD_derivatives(glmmr::MatrixField<VectorXd>& dD, glmmr::MatrixField<MatrixXd>& dA)
{
  A.setZero();
  Dvec.setZero();
  int idxlim;
...
VectorXd glmmr::nngpCovariance::log_gradient(const MatrixXd& umat, double& ll)
{
