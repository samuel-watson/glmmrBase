...
void glmmr::ModelOptim<modeltype>::set_direct_control(directd& op){
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_bobyqa_control(bobyqad& op){
  op.control.trace = trace;
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_newuoa_control(newuoad& op){
  op.control.trace = trace;
  op.control.rhobeg = control.rhobeg;
  op.control.rhoend = control.rhoend;
  op.control.npt = control.npt;
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_lbfgs_control(lbfgsd& op){
  op.control.trace = trace;
...
void glmmr::ModelOptim<modeltype>::ml_beta(){
...
template<typename modeltype>
template<class algo, typename>
void glmmr::ModelOptim<modeltype>::ml_theta(){  
...
template<typename modeltype>
template<class algo, typename>
void glmmr::ModelOptim<modeltype>::ml_all(){
  if(re.scaled_u_.cols() != re.u_.cols())re.scaled_u_.resize(NoChange,re.u_.cols());
  re.scaled_u_ = model.covariance.Lu(re.u_);
...
template<typename modeltype>
template<class algo, typename>
void glmmr::ModelOptim<modeltype>::laplace_ml_beta_u(){
  dblvec start = get_start_values(true,false,false);
...
template<typename modeltype>
template<class algo, typename>
void glmmr::ModelOptim<modeltype>::laplace_ml_theta()
{
...
template<typename modeltype>
template<class algo, typename>
void glmmr::ModelOptim<modeltype>::laplace_ml_beta_theta(){
  if(re.scaled_u_.cols() != re.u_.cols())re.scaled_u_.conservativeResize(NoChange,re.u_.cols());
  re.scaled_u_ = model.covariance.Lu(re.u_);
  dblvec start = get_start_values(true,true,false);  
  dblvec lower = get_lower_values(true,true,false);
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_direct_control(bool direct, double direct_range_beta, int max_iter, double epsilon, bool select_one, bool trisect_once, 
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::reset_fn_counter()
{
  fn_counter.first = 0;
  fn_counter.second = 0;
}
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_quantile(const double& q)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::ll_diff_variance(bool beta, bool theta)
{
...
template<typename modeltype>
std::pair<double,double> glmmr::ModelOptim<modeltype>::current_likelihood_values()
{
  return current_ll_values;
}

template<typename modeltype>
std::pair<double,double> glmmr::ModelOptim<modeltype>::u_diagnostic()
{
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_lbfgs_control(double g_epsilon, int past, double delta, int max_linesearch)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_all(const dblvec &par)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_laplace_beta_u(const dblvec &par)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_laplace_theta(const dblvec &par)
{
  // update_theta(par);
  // matrix.W.update();
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_laplace_beta_theta(const dblvec &par)
{
  auto start = par.begin();
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_beta(const dblvec& beta)
{
  model.linear_predictor.update_parameters(beta);
  double ll = log_likelihood();
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_beta_with_gradient(const VectorXd& beta, VectorXd& g)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_theta(const dblvec& theta)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_theta_with_gradient(const VectorXd& theta, VectorXd& g)
{
...
template<>
double glmmr::ModelOptim<bits_nngp>::log_likelihood_theta_with_gradient(const VectorXd& theta, VectorXd& g)
{
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood_laplace_beta_u_with_gradient(const VectorXd& x, VectorXd& g)
{
  MatrixXd v(Q(),1);
  v.col(0) = x.tail(Q());
...
template<>
double glmmr::ModelOptim<bits_hsgp>::log_likelihood_theta_with_gradient(const VectorXd& theta, VectorXd& g)
{
...
template<>
double glmmr::ModelOptim<bits_hsgp>::log_likelihood_theta(const dblvec& theta){
...
template<typename modeltype>
glmmr::ModelOptim<modeltype>::ModelOptim(modeltype& model_, 
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_bobyqa_control(int npt_, double rhobeg_, double rhoend_){
  control.npt = npt_;
...
template<typename modeltype>
int glmmr::ModelOptim<modeltype>::P() const {
  return model.linear_predictor.P();
}


template<typename modeltype>
int glmmr::ModelOptim<modeltype>::Q() const {
  return model.covariance.Q();
}

template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_beta(const dblvec &beta){
  bool update = true;
  if(beta_bounded)
  {
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_beta(const VectorXd &beta){
  bool update = true;
  if(beta_bounded)
  {
    for(int i = 0; i < beta.size(); i++)
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_theta(const dblvec &theta){
  model.covariance.update_parameters(theta);
  re.zu_ = model.covariance.ZLu(re.u_);
}

template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_theta(const VectorXd &theta){
  model.covariance.update_parameters(theta.array());
  re.zu_ = model.covariance.ZLu(re.u_);
}

template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_u(const MatrixXd &u_, bool append){
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_u(const MatrixXd &u_){
  int newcolsize = u_.cols();  
  if(u_.cols()!=re.u_.cols()){
    re.u_.resize(NoChange,newcolsize);
    re.zu_.resize(NoChange,newcolsize);
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood(bool beta) {
  ArrayXd xb(model.xb());
  int llcol = beta ? 0 : 1;
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::log_likelihood(){
  double ll = log_likelihood(true);
  return ll;
}

template<typename modeltype>
double glmmr::ModelOptim<modeltype>::full_log_likelihood(){
  double ll = log_likelihood();
  double logl = 0;
...
template<typename modeltype>
dblvec glmmr::ModelOptim<modeltype>::get_start_values(bool beta, bool theta, bool var)
{
  dblvec start;
  if(beta){
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_bound(const dblvec& bound, bool lower)
{
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::set_theta_bound(const dblvec& bound, bool lower)
{
  if(lower){
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::use_reml(bool reml)
{
  control.reml = reml;
}
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::generate_czz()
{
  CZZ.resize(Q(),Q());
  matrix.W.update();
...
template<typename modeltype>
dblvec glmmr::ModelOptim<modeltype>::get_lower_values(bool beta, bool theta, bool var, bool u)
{
#ifndef R_BUILD
...
template<typename modeltype>
dblvec glmmr::ModelOptim<modeltype>::get_upper_values(bool beta, bool theta, bool var, bool u){
  dblvec upper;
#ifndef R_BUILD
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::nr_beta(){
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::laplace_nr_beta_u(){
  matrix.W.update();
...
template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_var_par(const double& v){
  model.data.var_par = v;
  model.data.variance.setConstant(v);
}

template<typename modeltype>
void glmmr::ModelOptim<modeltype>::update_var_par(const ArrayXd& v){
  model.data.variance = v;
}

template<typename modeltype>
void glmmr::ModelOptim<modeltype>::calculate_var_par(){
...
template<typename modeltype>
double glmmr::ModelOptim<modeltype>::aic(){
  MatrixXd Lu = re.u();
...
template<typename modeltype>
ArrayXd glmmr::ModelOptim<modeltype>::optimum_weights(double N, 
                                                             VectorXd C,
                                                             double tol,
                                                             int max_iter){
#if defined(ENABLE_DEBUG) && defined(R_BUILD)
