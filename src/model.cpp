...
glmmr::Model<modeltype>::Model(const std::string& formula_,
...
template<typename modeltype>
void glmmr::Model<modeltype>::set_offset(const VectorXd& offset_){
  model.data.set_offset(offset_);
}

template<typename modeltype>
void glmmr::Model<modeltype>::set_weights(const ArrayXd& weights_){
...
template<typename modeltype>
void glmmr::Model<modeltype>::set_y(const VectorXd& y_){
  model.data.update_y(y_);
}

template<typename modeltype>
void glmmr::Model<modeltype>::update_beta(const dblvec &beta_){
  optim.update_beta(beta_);
}

template<typename modeltype>
void glmmr::Model<modeltype>::update_theta(const dblvec &theta_){
...
template<typename modeltype>
void glmmr::Model<modeltype>::reset_u(){
...
template<typename modeltype>
void glmmr::Model<modeltype>::update_u(const MatrixXd &u_, bool append){
...
template<typename modeltype>
void glmmr::Model<modeltype>::set_trace(int trace_){
  optim.trace = trace_;
  mcmc.trace = trace_;
  model.trace = trace_;
  if(trace_ > 0){
...
template<typename modeltype>
dblpair glmmr::Model<modeltype>::marginal(const MarginType type,
                                                 const std::string& x,
                                                 const strvec& at,
