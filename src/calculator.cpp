...
void glmmr::calculator::update_parameters(const dblvec& parameters_in){
...
VectorXd glmmr::calculator::linear_predictor(){
  int n = data.rows();
  VectorXd x(n);
...
glmmr::calculator& glmmr::calculator::operator= (const glmmr::calculator& calc){
...
MatrixXd glmmr::calculator::jacobian(){
  int n = data.rows();
#ifdef ENABLE_DEBUG
...
MatrixXd glmmr::calculator::jacobian(const VectorXd& extraData){
  int n = data.rows();
#ifdef ENABLE_DEBUG
  if(n==0)throw std::runtime_error("No data initialised in calculator");
#endif 
...
MatrixXd glmmr::calculator::jacobian(const MatrixXd& extraData){
  int n = data.rows();
...
MatrixMatrix glmmr::calculator::jacobian_and_hessian(const MatrixXd& extraData){
  int n = data.rows();
...
VectorMatrix glmmr::calculator::jacobian_and_hessian(){
...
double glmmr::calculator::get_covariance_data(const int i, const int j, const int fn){
...
dblvec glmmr::calculator::calculate(const int i, 
                                           const int j,
...
void glmmr::calculator::print_instructions() const {
...
void glmmr::calculator::print_names(bool print_data, bool print_parameters) const {
#ifdef R_BUILD
