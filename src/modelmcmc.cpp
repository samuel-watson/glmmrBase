...
glmmr::ModelMCMC<modeltype>::ModelMCMC(modeltype& model_, 
...
template<typename modeltype>
double glmmr::ModelMCMC<modeltype>::log_prob(const VectorXd &v){
...
template<typename modeltype>
VectorXd glmmr::ModelMCMC<modeltype>::new_proposal(const VectorXd& u0_,
                                                          bool adapt_, 
                                                          int iter_,
                                                          double runif_){
...
template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::sample(int warmup_,
                                                int nsamp_,
                                                int adapt_){
  boost::variate_generator<boost::mt19937, boost::normal_distribution<> >
  generator(boost::mt19937(time(0)),
...
template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::mcmc_sample(int warmup_,
                                                     int samples_,
                                                     int adapt_){
...
template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::mcmc_set_lambda(double lambda_){
  lambda = lambda_;
}

template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::mcmc_set_max_steps(int max_steps_){
  max_steps = max_steps_;
}

template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::mcmc_set_refresh(int refresh_){
  refresh = refresh_;
}

template<typename modeltype>
void glmmr::ModelMCMC<modeltype>::mcmc_set_target_accept(double target_){
