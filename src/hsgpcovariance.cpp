#include <glmmr/hsgpcovariance.hpp>


glmmr::hsgpCovariance::hsgpCovariance(const std::string& formula,
                                             const ArrayXXd& data,
...
glmmr::hsgpCovariance::hsgpCovariance(const glmmr::Formula& formula,
                                             const ArrayXXd& data,
                                             const strvec& colnames) : Covariance(formula, data, colnames),
                                             dim(this->re_cols_data_[0][0].size()),
                                             m(dim),
...
glmmr::hsgpCovariance::hsgpCovariance(const glmmr::hsgpCovariance& cov) : Covariance(cov.form_, cov.data_, cov.colnames_, cov.parameters_), 
...
void glmmr::hsgpCovariance::parse_hsgp_data(){
  for(int i = 0; i < dim; i++){
...
void glmmr::hsgpCovariance::update_approx_parameters(intvec m_, ArrayXd L_){
  m = m_;
  L_boundary = L_;
...
void glmmr::hsgpCovariance::update_approx_parameters(){
  total_m = glmmr::algo::prod_vec(m);
  this->Q_ = total_m;
  indices.conservativeResize(total_m,NoChange);
  Phi.conservativeResize(NoChange,total_m);
...
double glmmr::hsgpCovariance::spd_nD(int i){
  double wprod = 0;
  for(int d = 0; d < dim; d++) {
...
double glmmr::hsgpCovariance::d_spd_nD(int i, int par, bool sqrt_lambda)
{
  Array2d w;
...
ArrayXd glmmr::hsgpCovariance::phi_nD(int i){
...
MatrixXd glmmr::hsgpCovariance::D(bool chol, bool upper){
...
VectorXd glmmr::hsgpCovariance::sim_re(){
...
void glmmr::hsgpCovariance::update_parameters_extern(const dblvec& parameters){
  parameters_ = parameters;
  update_lambda();
};

void glmmr::hsgpCovariance::update_parameters(const dblvec& parameters){
  parameters_ = parameters;
  update_lambda();
};

void glmmr::hsgpCovariance::update_parameters(const ArrayXd& parameters){
...
MatrixXd glmmr::hsgpCovariance::ZL(){
  MatrixXd ZL = PhiSPD();
  return ZL;
}

MatrixXd glmmr::hsgpCovariance::ZL_deriv(int par)
{
...
MatrixXd glmmr::hsgpCovariance::LZWZL(const VectorXd& w){
...
MatrixXd glmmr::hsgpCovariance::ZLu(const MatrixXd& u){
  MatrixXd ZLu = glmmr::hsgpCovariance::ZL() * u;
  return ZLu;
}

MatrixXd glmmr::hsgpCovariance::Lu(const MatrixXd& u){
  MatrixXd ZLu = glmmr::hsgpCovariance::ZL() * u;
  return ZLu;
}

sparse glmmr::hsgpCovariance::ZL_sparse(){
  MatrixXd ZLmat = this->ZL();
  return SparseOperators::dense_to_sparse(ZLmat);
}

int glmmr::hsgpCovariance::Q() const{
  return this->Q_;
}

double glmmr::hsgpCovariance::log_likelihood(const VectorXd &u){
  double ll = 0;
...
double glmmr::hsgpCovariance::log_determinant(){
  double logdet = 0;
...
void glmmr::hsgpCovariance::set_function(bool squared_exp){
  sq_exp = squared_exp;
}

void glmmr::hsgpCovariance::gen_indices(){
...
void glmmr::hsgpCovariance::gen_phi_prod(){
  for(int i = 0; i < total_m; i++){
...
MatrixXd glmmr::hsgpCovariance::PhiSPD(bool lambda, bool inverse){
  MatrixXd pnew = Phi;
  if(lambda){
    if(!inverse){
...
ArrayXd glmmr::hsgpCovariance::LambdaSPD(){
  return Lambda;
}

void glmmr::hsgpCovariance::update_lambda(){
  for(int i = 0; i < total_m; i++){
    Lambda(i) = spd_nD(i);
  }
  L = PhiSPD(true,true);
